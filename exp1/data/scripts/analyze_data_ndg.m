...
tradeoff_datagraphs = {'participant' 'study_cb' 'list' 'study_instruction_condition' 'intact' 'OB' 'BG' 'F'};
...
arousal_data   = {'participant' 'study_cb' 'list' 'study_condition' 'arousal_ratings'};

%lme_test_data = {'id' 'list' 'cb' 'item_acc' 'sc_type' 'sc_valence' 'old_new' 'study_instruction' 'study_success_resp' 'study_arousal_resp' 'test_item_resp' 'original_scene' 'sc_image'};

lme_test_data = array2table(zeros(0,15), 'VariableNames',{'id' 'list' 'cb' 'item_acc' 'sc_type' 'sc_valence' 'scene_valence' 'old_new' 'study_instruction' 'study_success_resp' 'study_arousal_resp' 'test_item_resp' 'original_scene' 'sc_image' 'sc_code'});
...
for pari = 1:length(participant_list)
    ...
    LME_vars_keep = {'id' 'list' 'cb' 'item_acc' 'sc_type' 'sc_valence' 'scene_valence' 'old_new' 'study_instruction' 'study_success_resp' 'study_arousal_resp' 'test_item_resp' 'original_scene' 'sc_image' 'sc_code'};
    LME_data = test_data;
    LME_data = LME_data( :, ismember(LME_data.Properties.VariableNames, LME_vars_keep) );
    % Move them in the data table
    LME_data = movevars(LME_data,{'list' 'cb' 'item_acc' 'sc_type' 'sc_valence' 'scene_valence' 'old_new' 'study_instruction' 'study_success_resp' 'study_arousal_resp' 'test_item_resp' 'original_scene' 'sc_image' 'sc_code'},'After','id');
    ...
    sum(old_test_data_180.Intact);
    sum(old_test_data_180.Rearrange_OB);
    sum(old_test_data_180.Rearrange_BG);
    sum(old_test_data_180.Forget);
...
%         horzcat({participant},{study_cb},{list},{'failure'},{'F'},failure_F));
        ...
        tradeoff_datagraphs = vertcat(tradeoff_datagraphs, ...
        horzcat({participant},{study_cb},{list},{'decrease'},decrease_negative_OBBG, decrease_negative_OB, decrease_negative_BG, decrease_negative_F), ...
        horzcat({participant},{study_cb},{list},{'view_negative'},view_negative_OBBG,view_negative_OB,view_negative_BG,view_negative_F), ...
        horzcat({participant},{study_cb},{list},{'view_neutral'},view_neutral_OBBG,view_neutral_OB,view_neutral_BG,view_neutral_F), ...
        horzcat({participant},{study_cb},{list},{'success'},success_OBBG,success_OB,success_BG,success_F), ...
        horzcat({participant},{study_cb},{list},{'failure'},failure_OBBG,failure_OB,failure_BG,failure_F));
    ...
    % arousal ratings
        ...
        { participant  study_cb list  'decrease'    mean(arousal_ratings(old_trial & reappraisal & negative)) }, ...
        { participant  study_cb list 'success' mean(arousal_ratings(old_trial & reappraisal & success & negative)) }, ...
        { participant  study_cb list 'failure' mean(arousal_ratings(old_trial & reappraisal & failure & negative)) }, ...
        { participant study_cb list 'view_negative'    mean(arousal_ratings(old_trial & view & negative)) }, ...
        { participant study_cb list 'view_neutral'  mean(arousal_ratings(old_trial & view & neutral)) } );
...
arousal_data_wide = unstack(arousal_data_long,'arousal_ratings','study_condition');
arousal_data_wide.Properties.VariableNames(2:end) = ...
     cellfun(@(x) strcat(x,'_mean_arousal'), arousal_data_wide.Properties.VariableNames(2:end), 'UniformOutput', false); 
