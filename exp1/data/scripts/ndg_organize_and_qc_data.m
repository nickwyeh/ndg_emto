...
%     make_dirs({directories.par_data directories.par_beh});
    ...
    vars_to_remove = {'trials_thisIndex' 'OS' 'instruction_image_resp_keys' 'instruction_image_resp_rt' 'confirm_image_resp_keys' 'confirm_image_resp_rt'...
        ...
        'phases_thisRepN' 'phases_thisTrialN' 'phases_thisIndex' 'phases_ran' 'phases_order' 'trials_thisN' 'trials_thisindex'...
...
%     arousal_f_dec_neg = mean(study_data.study_arousal_rating(failure))
   ...
   writetable(test_data, test_beh_file, 'FileType','text','Delimiter','\t');
    ...
    % Copy variables arousal resp/rt, success resp/rt
    test_data.study_arousal_resp       = nan(size(test_data,1),1); % initialize study_resp vector.
    test_data.study_arousal_rt         = nan(size(test_data,1),1); % Initialize arousal rt vector
    test_data.study_success_resp       = repmat({'na'},size(test_data,1),1);
    test_data.study_success_rt         = nan(size(test_data,1),1); % Initialize success rt vector
    ...
    test_data.list                     = repmat({'na'},size(test_data,1),1); % initialize study list vector
